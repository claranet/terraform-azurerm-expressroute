locals {
  ...
  express_route_gateway_connection_name = coalesce(var.custom_express_route_gateway_connection_name, format("%s-%s-%s-%s-%s", "ergwc", var.client_name, var.location_short, var.environment, local.name_suffix))
}
